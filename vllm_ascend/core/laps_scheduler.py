...
class LapsRequestQueue(RequestQueue):
    ...
    def __init__(
        ...
        long_hard_deadline_mult: float = 2.0,
        ...
        #   * soft (>= long_max_wait_ms): the long is eligible for promotion but
        #     the token bucket rate-limits it, so a backlog of aged longs cannot
        #     flip the queue into long-first and starve shorts.
        #   * hard (>= hard bound): the long is promoted unconditionally,
        #     bypassing the bucket, making the worst-case wait a true upper bound.
        # With reservation == 0 the bucket never refills, so there is no useful
        # soft phase; the hard bound collapses to long_max_wait_ms (pure deadline
        # aging). With reservation > 0 the hard bound is long_max_wait_ms times
        # this multiplier, leaving [soft, hard) for bucket-smoothed promotion.
        self.long_hard_deadline_mult = long_hard_deadline_mult
        ...
        # Expected range [0.0, 1.0] (enforced by LapsConfig).
    ...
    def _long_hard_deadline_ms(self) -> float:
        ...
        deadline aging); otherwise long_max_wait_ms * long_hard_deadline_mult.
        """
        if self.long_token_reservation <= 0.0:
            return self.long_max_wait_ms
        return self.long_max_wait_ms * self.long_hard_deadline_mult
    ...
    def _debug_select(self, event: str) -> None:
        ...
        if self._debug_logging_enabled:
            self._debug_state(
                event,
                extra=f"bucket={self._long_bucket:.0f} "
    ...
    def _select_schedulable_queue(self) -> RequestQueue | None:
        ...
        if self._immediate_queue:
            return self._immediate_queue
        if self._long_queue:
            # Read the long-queue head age once and reuse it for both the soft
            # and hard aging checks (avoids a second peek + monotonic()).
            wait_ms = self._long_head_wait_ms()
            if wait_ms is not None and wait_ms >= self.long_max_wait_ms:
                # Token-bucket reservation: aged-long admissions are rate-limited
                # to the reservation fraction of token throughput. When the short
                # queue is non-empty, only promote long while the bucket has
                # credit; when it is empty, always allow long (stall avoidance).
                ...
                if self._long_bucket > 0.0:
                    ...
                    return self._long_queue
                if wait_ms >= self._long_hard_deadline_ms():
                    # Hard deadline: the long has waited past its hard bound.
                    # Promote it unconditionally (bypassing the bucket) so the
                    # worst-case wait stays a true upper bound even when the
                    # reservation rate is too low to drain it via the soft phase.
                    self._debug_select("aged-long_selected_hard_deadline")
                    return self._long_queue
                # Bucket exhausted, hard deadline not reached, short queue
                # non-empty: prefer short (fall through below).
        ...
        if count_as_removal:
            ...
            if aged:
                ...
                if jumped_shorts:
                    ...
                    # Hard-deadline bypass: selected past the hard bound while the
                    # bucket was empty, so soft-phase smoothing alone would not
                    # have admitted it. Such emergency promotions do NOT charge
                    # the bucket, so a burst cannot drive it into unbounded
                    # negative debt (which would otherwise block soft promotions
                    # for many steps afterwards). Only soft-phase promotions
                    # (taken while the bucket had credit) are charged, so the
                    # bucket dips below zero by at most one chunk before refill.
                    hard_bypass = (
                        self._long_bucket <= 0.0
                ...
                if self._debug_logging_enabled:
                    self._debug_state(
                        ...
                        request=request,
                        queue=queue,
                        extra=f"scheduled_tokens={scheduled_tokens} charged={charged} "
...
class LapsSchedulerMixin:
    ...
    def _init_laps_waiting_queue(
        ...
        if self.policy != SchedulingPolicy.FCFS:
            ...
            logger.debug(
                "LAPS scheduling supports only FCFS policy (current=%s); "
        ...
        self.waiting = LapsRequestQueue(
            ...
            long_hard_deadline_mult=laps_config.long_hard_deadline_mult,
