...
class BenchmarkRunner:
    def __init__(
        ...
        self.rate = rate  # requests per second (None = closed-loop mode)
        self.results = []
    ...
    def _run_single_request(self, req: Dict[str, Any], label: str) -> Dict[str, Any]:
        """发送单个请求并打印进度（用于 open-loop 模式）"""
        result = self.send_request_stream(req)
        self._print_progress(result, label)
        return result
    ...
    def run_benchmark(self, requests_by_file: Dict[str, List[Dict[str, Any]]]):
        ...
        两种模式：
        - rate > 0: open-loop，按泊松过程发请求
        - rate == None: closed-loop，固定并发文件数
        """
        total_requests = sum(len(reqs) for reqs in requests_by_file.values())
        ...
        # ========== Open-loop 模式：泊松过程请求速率 ==========
        if self.rate is not None:
            avg_interval = 1.0 / self.rate
            print(f"Mode: open-loop (Poisson process), rate={self.rate:.2f} req/s (avg_interval={avg_interval:.3f}s)")
            print(f"Threads: {self.max_workers} (max concurrent)")
            print("Press Ctrl+C to stop and show current results")
            print("-" * 80)

            # 把所有请求按文件顺序展平
            all_requests: List[tuple[str, Dict[str, Any], int]] = []
            for filename, requests in requests_by_file.items():
                for idx, req in enumerate(requests):
                    all_requests.append((filename, req, idx + 1))
            ...
            # Open-loop: 使用足够大的 max_workers，避免线程池成为瓶颈
            # 实际并发由发送速率控制，而非线程池限制
            with ThreadPoolExecutor(max_workers=None) as executor:
                futures = {}

                while request_index < len(all_requests) and not self.interrupted:
                    current_time = time.time()

                    # 收集已完成的结果
                    done, _ = wait(futures.keys(), timeout=0.01, return_when=FIRST_COMPLETED)
                    for future in done:
                    ...
                    # 如果到达提交时间，提交新请求（纯 open-loop，只控制速率）
                    if current_time >= next_submit_time:
                        filename, req, req_idx = all_requests[request_index]
                        label = f"{filename}[{req_idx}]"
                        future = executor.submit(self._run_single_request, req, label)
                        futures[future] = label
                        request_index += 1

                        # 使用泊松过程计算下次提交时间
                        interval = np.random.exponential(avg_interval)
                        next_submit_time = current_time + interval

                    # 如果没有需要立即处理的任务，短暂休眠
                    else:
                        sleep_time = max(0, next_submit_time - current_time)
                        if sleep_time > 0:
                ...
                # 等待剩余任务完成
                if futures:
                    print(f"\nWaiting for {len(futures)} in-flight requests to complete...")
            ...
            print(f"\nSubmitted: {request_index}, Completed: {completed}")
            return self.results

        # ========== Closed-loop 模式：固定并发文件数 ==========
        print(f"Mode: closed-loop, threads: {self.max_workers} (concurrent files)")
        print("Press Ctrl+C to stop and show current results")
        print("-" * 80)
        ...
        if num_files == 1:
            ...
            # 多文件：使用 num_workers 控制并发文件数（每个文件一个线程）
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Request rate in req/s (open-loop mode with Poisson process). Inter-arrival times follow an exponential distribution. If not set, uses closed-loop mode with fixed concurrency.",
    )
    parser.add_argument(
        "--session-count",
        type=int,
