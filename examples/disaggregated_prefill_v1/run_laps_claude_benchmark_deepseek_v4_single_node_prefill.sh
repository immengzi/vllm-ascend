...
# Run a real Claude-trace benchmark for DeepSeek-V4 on a single-node Prefill-only deployment.
#
...
RESULT_DIR="${RESULT_DIR:-/vllm-workspace/bench_results/deepseek_v4_claude_prefill_only_$(date +%Y%m%d_%H%M%S)}"
CASE_VARIANTS="${CASE_VARIANTS:-off t4096}"
...
PREFILL_PORT="${PREFILL_PORT:-6750}"
PREFILL_KV_PORT="${PREFILL_KV_PORT:-30050}"

PREFILL_DP_SIZE="${PREFILL_DP_SIZE:-1}"
PREFILL_TP_SIZE="${PREFILL_TP_SIZE:-8}"
# These values are only used to describe the remote decode-side topology in
# kv-transfer metadata. This script never launches a decode service.
KV_DECODE_DP_SIZE="${KV_DECODE_DP_SIZE:-1}"
KV_DECODE_TP_SIZE="${KV_DECODE_TP_SIZE:-8}"
...
append_summary() {
  ...
  python3 - "$case_name" "$variant_name" "$threshold" "$measured_json" "$prefill_before" "$prefill_after" "$prepared_dataset_dir" "$dataset_manifest" "$prepared_session_count" "$prepared_request_count" >> "${RESULT_DIR}/summary.csv" <<'PY'
import json
import math
import re
import sys
...
start_prefill() {
  ...
  kv_transfer_config_json="{\"kv_connector\":\"MooncakeHybridConnector\",\"kv_role\":\"kv_producer\",\"kv_port\":\"${PREFILL_KV_PORT}\",\"engine_id\":\"0\",\"kv_connector_extra_config\":{\"prefill\":{\"dp_size\":${PREFILL_DP_SIZE},\"tp_size\":${PREFILL_TP_SIZE}},\"decode\":{\"dp_size\":${KV_DECODE_DP_SIZE},\"tp_size\":${KV_DECODE_TP_SIZE}}}}"
  ...
  (
    ...
    # LAPS is configured via additional_config.laps_config (see below). The
    # laps_wait_window_ms tuple field carries long_max_wait_ms; long_token_reservation
    # carries the reservation fraction.
    if [ "${laps_threshold}" = "off" ]; then
      laps_config_json='"laps_config":{"enabled":false}'
    else
      laps_config_json="\"laps_config\":{\"enabled\":true,\"threshold\":${laps_threshold},\"long_max_wait_ms\":${laps_wait_window_ms},\"long_token_reservation\":${long_token_reservation},\"stats_log_interval_s\":${LAPS_STATS_LOG_INTERVAL_S}}"
    fi
    ...
    exec setsid vllm serve "${MODEL_PATH}" \
      ...
      --additional-config '{"ascend_compilation_config":{"enable_npugraph_ex":true,"enable_static_kernel":false},"enable_cpu_binding":true,"multistream_overlap_shared_expert":false,"multistream_dsa_preprocess":false,"recompute_scheduler_enable":true,'"${laps_config_json}"'}' \
...
run_case() {
  ...
  if [ "${RUN_WARMUP}" = "1" ]; then
    ...
    run_claude_bench "${case_name}" "warmup" "${WARMUP_TIMEOUT}" "${warmup_output_dir}" "${warmup_json}" "${rate}"
  ...
  run_claude_bench "${case_name}" "measured" "${MEASURE_TIMEOUT}" "${measured_output_dir}" "${measured_json}" "${rate}"
...
main() {
  ...
  for rate in ${BENCH_RATES:-unlimited}; do
