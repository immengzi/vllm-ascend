...
start_prefill() {
  ...
  (
    ...
    # LAPS is configured via additional_config.laps_config and requires the
    # recompute scheduler. long_prefill_cap / short_reserved_ratio are from a
    # removed design and are ignored. wait_window_ms carries long_max_wait_ms.
    # The additional-config flag is only added when LAPS is enabled, so the
    # "off" baseline keeps the plain scheduler.
    if [ "${laps_threshold}" = "off" ]; then
      laps_additional_config=()
    else
      laps_additional_config=(--additional-config "{\"recompute_scheduler_enable\":true,\"laps_config\":{\"enabled\":true,\"threshold\":${laps_threshold},\"long_max_wait_ms\":${wait_window_ms},\"stats_log_interval_s\":${LAPS_STATS_LOG_INTERVAL_S:-0}}}")
    fi
    exec vllm serve "${MODEL_PATH}" \
      ...
      ${laps_additional_config[@]+"${laps_additional_config[@]}"} \
      --no-async-scheduling \
...
start_decode() {
  ...
  (
    ...
    unset VLLM_ASCEND_LAPS_SCHEDULING VLLM_ASCEND_LAPS_THRESHOLD VLLM_ASCEND_LAPS_LONG_MAX_WAIT_MS
    exec vllm serve "${MODEL_PATH}" \
      --host "${DECODE_HOST}" \
      --port "${DECODE_PORT}" \
      --served-model-name "${SERVED_MODEL_NAME}" \
...
run_case() {
  ...
  log "Case config: variant=${variant_name}, threshold=${threshold}, long_max_wait_ms=${wait_window_ms:-0}, long_prefill_cap=${long_prefill_cap:-0}, short_reserved_ratio=${short_reserved_ratio:-0}, request_rate=${request_rate}"
