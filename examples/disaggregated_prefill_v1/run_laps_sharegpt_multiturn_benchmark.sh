...
start_prefill() {
  ...
  (
    ...
    if [ "${laps_threshold}" = "off" ]; then
      unset VLLM_ASCEND_LAPS_SCHEDULING VLLM_ASCEND_LAPS_THRESHOLD VLLM_ASCEND_LAPS_LONG_MAX_WAIT_MS VLLM_ASCEND_LAPS_LONG_PREFILL_CAP VLLM_ASCEND_LAPS_SHORT_RESERVED_RATIO VLLM_ASCEND_LAPS_STATS_LOG_INTERVAL_S
    else
      ...
      export VLLM_ASCEND_LAPS_LONG_MAX_WAIT_MS="${wait_window_ms}"
