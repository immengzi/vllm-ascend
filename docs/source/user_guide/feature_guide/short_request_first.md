...
Add `short_request_first_config` to `additional_config`. It is supported by FCFS synchronous and asynchronous scheduling in ordinary deployments, on PD-disaggregated prefill (P) nodes, and in PD-mixed deployments. It does not require `recompute_scheduler_enable`:
...
ShortRequestFirst changes only the waiting-queue policy. It requires FCFS scheduling and supports both synchronous and asynchronous scheduling. It is not supported with the recompute scheduler, dynamic-batch scheduling, or on PD-disaggregated D nodes (`kv_role='kv_consumer'`).
