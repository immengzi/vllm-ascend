...
ShortRequestFirst changes only the FCFS waiting-queue policy. It supports synchronous and asynchronous scheduling, subject to upstream async-scheduling compatibility requirements. It is not supported with batch-job-aware scheduling, profiling-chunk scheduling, or on PD-disaggregated D nodes (`kv_role='kv_consumer'`).
