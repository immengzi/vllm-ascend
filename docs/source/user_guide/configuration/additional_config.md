...
LAPS-style length-aware prefill scheduling (triple-queue + two-tier anti-starvation aging). Only takes effect together with `recompute_scheduler_enable=true` and the FCFS scheduler policy.
...
| `long_max_wait_ms`       | float | `0.0`   | Anti-starvation aging bound for long prefills, in milliseconds. A long request that has waited longer than this is promoted ahead of short prefills (rate-limited soft phase, then unconditional past the hard deadline). `0` disables aging (strict short-priority). |
| `long_token_reservation` | float | `0.0`   | Average fraction of token throughput reserved for admitting aged-long prefills ahead of waiting shorts during the soft aging phase (token-bucket smoothing). Clamped to `[0.0, 1.0]`. `0` reduces aging to a pure deadline at `long_max_wait_ms`. |
...
| `long_hard_deadline_mult`| float | `2.0`   | Multiplier on `long_max_wait_ms` for the hard deadline (unconditional promotion). Only takes effect when `long_token_reservation > 0`. Must be `>= 1.0`. |
