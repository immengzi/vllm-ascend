...
| `long_token_reservation` | float | `0.0`   | Average fraction of token throughput reserved for admitting aged-long prefills ahead of waiting shorts (token-bucket smoothing) — the only channel that promotes aged longs. Clamped to `[0.0, 1.0]`. Must be `> 0` whenever `long_max_wait_ms > 0`. |
