...
| `short_request_first_config` | dict | `{}` | Configuration options for synchronous ShortRequestFirst waiting admission on PD prefill (P) and PD-mixed nodes. Independent of `recompute_scheduler_enable`. |
...
ShortRequestFirst is a synchronous waiting-queue policy for PD prefill (P), PD-mixed (including balance scheduling), and ProfilingChunk deployments. It requires FCFS and is not supported with async scheduling, BatchJobAware scheduling, or PD-disaggregated D nodes. It is independent of the recompute scheduler. See [ShortRequestFirst Prefill Scheduling](../feature_guide/short_request_first.md) for usage, behavior, and tuning guidance.
