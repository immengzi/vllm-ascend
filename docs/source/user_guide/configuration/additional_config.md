...
ShortRequestFirst is a waiting-queue policy for FCFS scheduling on ordinary prefill, PD-prefill, and PD-mixed paths. It supports synchronous and asynchronous scheduling, subject to upstream async-scheduling compatibility requirements. It does not support batch-job-aware, profiling-chunk, or PD-disaggregated D-node scheduling. See [ShortRequestFirst Prefill Scheduling](../feature_guide/short_request_first.md) for usage, behavior, and tuning guidance.
