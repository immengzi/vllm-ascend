...
@pytest.mark.parametrize("enabled", [False, True])
def test_sync_scheduler_only_installs_queue_when_enabled(enabled):
    vllm_config = SimpleNamespace()
    ascend_config = SimpleNamespace(
        short_request_first_config=SimpleNamespace(
            enabled=enabled,
    ...
    with (
        patch.object(Scheduler, "__init__", initialize_scheduler),
        patch("vllm_ascend.ascend_config.init_ascend_config", return_value=ascend_config),
    ):
        scheduler = ShortRequestFirstScheduler(vllm_config=vllm_config)
